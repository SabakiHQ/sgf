...
t.test('parseDates', t => {
    ...
    t.test('should handle short-hand notation', t => {
        ...
        t.deepEqual(sgf.parseDates('1996-05,1997'), [
            [1996, 5], [1997]
        ])
        t.deepEqual(sgf.parseDates('1996-05-06,07,08'), [
            [1996, 5, 6], [1996, 5, 7], [1996, 5, 8]
        ])
        t.deepEqual(sgf.parseDates('1996,1997'), [
            [1996], [1997]
        ])
...
t.test('stringifyDates', t => {
    ...
    t.test('should be inverse to parseDates', t => {
        t.deepEqual(sgf.parseDates(sgf.stringifyDates([
            [1996, 5], [1996, 6]
        ])), [
            [1996, 5], [1996, 6]
        ])
        ...
        t.equal(sgf.stringifyDates(sgf.parseDates('1996-05-06,07,08')), '1996-05-06,07,08')
        t.equal(sgf.stringifyDates(sgf.parseDates('1996,1997')), '1996,1997')
        t.equal(sgf.stringifyDates(sgf.parseDates('1996-12-27,28,1997-01-03,04')), '1996-12-27,28,1997-01-03,04')
...
t.test('parseVertex', t => {
    ...
    t.test('should be left inverse to stringifyVertex', t => {
        ...
        tests.forEach(test => t.deepEqual(sgf.parseVertex(sgf.stringifyVertex(test)), test))
...
t.test('stringifyVertex', t => {
    ...
    t.test('should be left inverse to parseVertex', t => {
        ...
        tests.forEach(test => t.equal(sgf.stringifyVertex(sgf.parseVertex(test)), test))
...
t.test('parseCompressedVertices', t => {
    ...
    t.test('should handle compressions', t => {
        t.deepEqual(sgf.parseCompressedVertices('aa:bb'), [[0, 0], [0, 1], [1, 0], [1, 1]])
        t.deepEqual(sgf.parseCompressedVertices('bb:aa'), [[0, 0], [0, 1], [1, 0], [1, 1]])
